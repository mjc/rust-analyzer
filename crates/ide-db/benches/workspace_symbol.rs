...
use salsa::Setter;
use syntax::{
    Edition, GreenNode, SourceFile, SyntaxNode, TextRange, TextSize,
...
fn ast_id_map(source: SyntaxNode) -> span::AstIdMap {
    black_box(span::AstIdMap::from_source(&source))
}
...
fn tiny_ast_id_maps(sources: Vec<SyntaxNode>) -> Vec<span::AstIdMap> {
    black_box(sources.iter().map(span::AstIdMap::from_source).collect())
}
...
fn setup_syntax_cursor_identity() -> (SyntaxNode, SyntaxNode) {
    let green = setup_nested_ast_id_map().green().to_owned();
    (SyntaxNode::new_root(green.clone()), SyntaxNode::new_root(green))
}

#[library_benchmark(config = LibraryBenchmarkConfig::default().tool(Dhat::default()))]
#[bench::nested_modules(setup_nested_ast_id_map())]
fn nested_ast_id_map(source: SyntaxNode) -> span::AstIdMap {
    black_box(span::AstIdMap::from_source(&source))
}
...
fn setup_green_nodes_with_leaf_children() -> Vec<GreenNode> {
    let kind = SyntaxKind(0);
    (0..16_384).map(|_| GreenNode::new(kind, [GreenToken::new(kind, "token").into()])).collect()
}

fn setup_checkpointed_green_nodes_with_leaf_children() -> Vec<GreenNode> {
    let kind = SyntaxKind(0);
    let token = GreenToken::new(kind, "token");
    (0..4096).map(|_| GreenNode::new(kind, (0..8).map(|_| token.clone().into()))).collect()
}
...
#[bench::checkpointed_leaf_children(setup_checkpointed_green_nodes_with_leaf_children())]
...
fn construct_green_nodes((kind, token): (SyntaxKind, GreenToken)) -> usize {
    let nodes = (0..4096)
        .map(|_| GreenNode::new(kind, (0..4).map(|_| token.clone().into())))
        .collect::<Vec<_>>();
...
#[library_benchmark(config = LibraryBenchmarkConfig::default().tool(Dhat::default()))]
#[bench::node_and_token(setup_green_refcount_pair())]
fn clone_borrowed_green_refs(pair: (GreenNode, GreenToken)) -> usize {
    let (node, token) = pair;
    black_box(
        (0..4096)
            .map(|_| {
                ...
                let node = black_box(node.to_owned());
                let token = black_box(token.to_owned());
                let len =
                    u32::from(black_box(node.text_len())) + u32::from(black_box(token.text_len()));
                drop(node);
                drop(token);
...
fn fill_shared_parse_cache(sources: Vec<String>) -> usize {
    black_box(
        sources
            ...
            .map(|source| {
                u32::from(
                    SourceFile::parse(black_box(source), Edition::CURRENT)
                        .syntax_node()
                        .text_range()
                        .len(),
                ) as usize
...
fn setup_shared_attr_syntax_nodes() -> Vec<SyntaxNode> {
    ...
    (0..1024).map(|_| SourceFile::parse(source, Edition::CURRENT).syntax_node().clone()).collect()
}
